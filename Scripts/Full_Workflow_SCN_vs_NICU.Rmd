...
ordination_plots(ps2, "Nursery_Discharge_Weight", "PCoA", "bray")
...
```{r, warning=F, message=F}
deseq_model <- phyloseq_to_deseq2(ps3, ~ Feeding_Type + NEC + Sepsis + Mode_of_Delivery +  
                   Neonatal_Antibiotics + Chorioamnionitis + Preeclampsia + ROP + Primary_Group + Batch) %>% 
    calc_geo_means() %>% 
    deseq_filter() %>% 
    DESeq(fitType = "local", test = "Wald")
...
## envfit
```{r}
ordination_distance <- ordinate(ps2, method="NMDS", distance="bray")
envdat.phy <- ps2 %>% 
  sample_data() %>%
  unclass() %>% 
  as.data.frame() %>% 
...
envfit(ordination_distance, envdat.phy, permutations = 999) 
...
```{r}
sjPlot::plot_model(global2, vline.color = "red", sort.est = TRUE, title = "GLM for Shannon diversity", type="pred") 
```
