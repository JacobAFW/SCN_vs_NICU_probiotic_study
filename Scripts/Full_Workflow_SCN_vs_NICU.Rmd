...
```{r, warning=F, message=F, fig.cap="Tree plots exploring the agglomeration of taxa at the genus level.", eval=F}
grid.arrange(nrow = 1, 
...
**Change `Type`.**
```{r, warning=F, message=F, fig.cap="Tree plot exploring the normalised distribution of taxa between admission and discharge samples.", eval=F}
...
ps4 %>% 
  subset_samples(Feeding_Type == "Formula") %>% 
...
ps4 %>% 
  subset_samples(Primary_Group == "SCN") %>% 
samples_with_taxa(., "Genus", "Bifidobacterium")

ps4 %>% 
  subset_samples(Primary_Group == "NICU") %>% 
samples_with_taxa(., "Genus", "Lactobacillus")
...
### Statistical test: PERMANOVA.
 ...
 - Homogeneity Condition
  ...
  Primary_Group + Feeding_Type + NEC + Sepsis + 
                      Mode_of_Delivery +  Neonatal_Antibiotics + Chorioamnionitis + 
                      Preeclampsia + ROP + Batch
...
ordination_plots(ps2.TSS, "Nursery_Discharge_Weight", "PCoA", "bray")
...
ps_otu <- data.frame(otu_table(ps2.TSS))
ps_metadata <- data.frame(sample_data(ps2.TSS))
permanova <- adonis(ps_otu ~Neonatal_Antibiotics, data = ps_metadata, method = "bray") 
permanova <- tableGrob(as.data.frame(permanova$aov.tab)) %>% 
  annotate_figure(fig.lab = "PERMANOVA", fig.lab.face = "bold", fig.lab.size = 15)

# homogeneity condition
dist <- vegdist(ps_otu)
homogeneity <- as.data.frame(anova(betadisper(dist, ps_metadata$Neonatal_Antibiotics))) %>% 
  tableGrob() %>% 
  annotate_figure(fig.lab = "Homogeneity Condition", fig.lab.face = "bold", fig.lab.size = 15)
...
### Explore taxonmic distribution.

```{r}
plot_bar_auto <- function(ps, taxonomy){
plot_bar(ps, fill = taxonomy) + 
    facet_wrap(~Neonatal_Antibiotics, scales = "free_x") + 
    labs(title = paste0("Level:", taxonomy), y = "Abundance") + 
    theme(legend.position = "bottom", legend.title = element_blank(), 
    axis.title.x = element_blank(), axis.text.x = element_blank(), 
    axis.ticks = element_blank())
...
```{r, eval=F}
grid.arrange(ordination_plots(ps2.TSS, "Primary_Group", "PCoA", "bray"), 
             ordination_plots(ps2.TSS, "Feeding_Type", "PCoA", "bray"), 
             ncol=2)
```
...
sample_data(ps3) %>% 
  unclass %>% 
  as.data.frame() %>% 
  select(Feeding_Type, NEC, Sepsis, Mode_of_Delivery, Neonatal_Antibiotics, Chorioamnionitis, Preeclampsia, ROP, Primary_Group, Batch) %>% 
  centre_and_scale() %>% 
  corvif()
```

Maybes: Antenatal_Infections, Antenatal_Antibiotics,
...
```{r, warning=F, message=F}
phyloseq_to_deseq2(ps3, ~ Feeding_Type + NEC + Sepsis + Mode_of_Delivery +  
                   Neonatal_Antibiotics + Chorioamnionitis + Preeclampsia + ROP + Primary_Group + Batch) %>% 
    calc_geo_means() %>% 
    deseq_filter(10, 10) %>% 
...
phyloseq_to_deseq2(ps3, ~ Primary_Group + Feeding_Type + NEC + Sepsis + Mode_of_Delivery + Neonatal_Antibiotics + Chorioamnionitis +
...
**DECIDE IN WHICH MODEL TO USE - out of the 2 chunks below.**
```{r, warning=F, message=F}
deseq_model <- phyloseq_to_deseq2(ps3, ~ Primary_Group + Feeding_Type + NEC + Sepsis + Mode_of_Delivery + Neonatal_Antibiotics + Chorioamnionitis +
          Preeclampsia + ROP + Batch + Diabetes + Antenatal_Antibiotics) %>% 
          calc_geo_means() %>% 
          deseq_filter(10, 10) %>% 
          DESeq(fitType = "local", test = "Wald")
...
deseq_model %>% 
  get_deseq_res_cat("Primary_Group", "NICU", "SCN") %>% remove_rownames() %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Feeding_Type", "Formula", "Breastmilk") %>% remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("NEC", "Yes", "No") %>% remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Sepsis", "Yes", "No") %>% remove_rownames()) %>% 
   rbind(deseq_model %>% get_deseq_res_cat("Mode_of_Delivery", "Vaginal", "Cesarean") %>% remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Diabetes", "Yes", "No") %>% remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Neonatal_Antibiotics", "Yes", "No") %>% remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Preeclampsia", "Yes", "No") %>% remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Chorioamnionitis", "Yes", "No") %>% remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Batch", "Run1", "Run2") %>% remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("ROP", "Yes", "No") %>% remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Antenatal_Antibiotics", "Yes", "No") %>% remove_rownames()) %>% View()
```
...
# Mixed effects modelling with PERMANOVA
...
permanova <- adonis(ps_otu ~Primary_Group + Feeding_Type + NEC + Sepsis + 
                      Mode_of_Delivery +  Neonatal_Antibiotics + Chorioamnionitis + 
                      Preeclampsia + ROP + Batch, data = ps_metadata, method = "bray", permutations = 10000) 
permanova <- tableGrob(as.data.frame(permanova$aov.tab)) %>% 
  annotate_figure(fig.lab = "PERMANOVA", fig.lab.face = "bold", fig.lab.size = 15)
...
data.frame() %>% 
vegdist() %>% 
betadisper(data.frame(sample_data(ps2.TSS))$Batch) %>% 
anova()
```
...
ordination_distance <- ordinate(
    ...
    method = "NMDS",
    distance = "bray",
    formula = ~ Primary_Group+ Feeding_Type+ NEC+ Sepsis+ Mode_of_Delivery+ Neonatal_Antibiotics+ Chorioamnionitis+
          Preeclampsia+ ROP+ Batch+ Gestational_Age_at_Birth+ Prolonged_Membrane_Rupture+ Diabetes+ Birth_Weight+ 
          Nursery_Discharge_Weight+ Antenatal_Infections+ Antenatal_Antibiotics)

 envdat.phy <- ps2.TSS %>% 
  sample_data() %>%
  unclass() %>% 
  as.data.frame() %>% 
  select(Primary_Group, Feeding_Type, NEC, Sepsis, Mode_of_Delivery, Neonatal_Antibiotics, Chorioamnionitis,
          Preeclampsia, ROP, Batch, Gestational_Age_at_Birth, Prolonged_Membrane_Rupture, Diabetes, Birth_Weight, 
          Nursery_Discharge_Weight, Antenatal_Infections, Antenatal_Antibiotics) 

envfit_object <- envfit(ordination_distance, envdat.phy, permutations = 10000, na.rm = T) 
```
...
envfit_object$vectors$r %>% 
  ...
  rownames_to_column("Variable") %>%  
  rename("r2" = ".") %>% 
  rows_update(., tibble(Variable = "Sepsis", r2 = 0.1)) %>% # changed to 0.1 so it fits on the plot if we create limits
  left_join(
  ...
  scale_y_discrete(labels = c("Prolonged_Membrane_Rupture" = "Prolonged membrane rupture", 
                              "Primary_Group" = "Probiotic group",
                              "Nursery_Discharge_Weight" = "Nursery discharge weight",
                              ...
                              "Antenatal_Infections" = "Antenatal infections",
                              "Antenatal_Antibiotics" = "Antenatal antibiotics"))
  
```

### Only variables included in other models
...
### Only variables previously found to have  significant association 
...
ordination_distance <- ordinate(
    ...
    distance = "bray",
    formula = ~ Primary_Group + Feeding_Type + NEC + Sepsis + Mode_of_Delivery + Neonatal_Antibiotics + Chorioamnionitis +
          Preeclampsia + ROP + Batch + Gestational_Age_at_Birth + Diabetes + Antenatal_Antibiotics)
...
# ggsave("envfit.png", dpi = 1000, height = 5, width = 7)

```

## Calculate major contributors
...
ps_otu <- data.frame(otu_table(ps2.TSS))
ps_metadata <- data.frame(sample_data(ps2.TSS))
permanova <- adonis(ps_otu ~Primary_Group + Feeding_Type + NEC + Sepsis + Mode_of_Delivery + Neonatal_Antibiotics + Chorioamnionitis +
          Preeclampsia + ROP + Batch + Gestational_Age_at_Birth + Diabetes + Antenatal_Antibiotics, data = ps_metadata, method = "bray")  
...
major_contributors(ps2.TSS, "Primary_Group") %>% # there are a few ASVs from the same genus so we need to summarise to imporve the plot
  filter(top.coef > 0) %>% 
  group_by(Genus) %>% 
...
grid.arrange(
  ...
  geom_col() +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 10), limits = c(0, 0.1)) +
  theme(axis.line =  element_line(size = .5), panel.background = element_blank(), legend.position = c(.75,.5)) + 
  scale_fill_continuous(type = "viridis") +
  scale_y_discrete(labels = c("Primary_Group" = "Probiotic group",
                              "Neonatal_Antibiotics" = "Neonatal antibiotics",
                              "Mode_of_Delivery" = "Mode of delivery",
...
**REMOVE?**
```{r, warning=F, message=F}
bray_not_na <- phyloseq::distance(physeq = ps2.TSS, method = "bray")
...
cap_plot <- plot_ordination(
  ...
  ordination = cap_ord, 
    ...
    aes(shape = Primary_Group) + 
    geom_point(aes(colour = Primary_Group), alpha = 0.4, size = 4) + 
...
```{r, warning=F, message=F, results='hide'}
global <- lme4::glmer(Shannon ~ Feeding_Type + Sepsis + Mode_of_Delivery + Preeclampsia + ROP + Primary_Group + Batch + (1|URN), data = glm_data) 

global <- lme4::glmer(Shannon ~ Feeding_Type + Sepsis + Mode_of_Delivery + Preeclampsia + ROP + Primary_Group + Batch + (1|URN), data = (glm_data %>% filter(Shannon > 0)), family = Gamma(link = "log"))

global <- lme4::glmer(Shannon ~ Primary_Group + Feeding_Type + NEC + Sepsis + Mode_of_Delivery + Neonatal_Antibiotics + Chorioamnionitis +
          Preeclampsia + ROP + Batch + Prolonged_Membrane_Rupture + Diabetes + 
          Nursery_Discharge_Weight + Antenatal_Infections + Antenatal_Antibiotics + (1|URN), data = glm_data %>% filter(Shannon > 0), family = Gamma(link = "log")) 
```
...
**The gamma model with all non-covariate variables is the best model.**
...
global2 <- lme4::glmer(Shannon ~ Feeding_Type + Sepsis + Primary_Group + Batch + (1|URN), data = glm_data) 
  
dfun(drop1(global2))
```

## lmerTest for statistical significance
```{r}
lmerTest::lmer(Shannon ~ Feeding_Type + Sepsis + Primary_Group + Batch + (1|URN), data = glm_data) %>% summary()
```
...
```{r}
sjPlot::plot_model(global2, vline.color = "red", sort.est = TRUE, title = "GLM for Shannon diversity") 
```

```{r}
sjPlot::plot_model(global2, vline.color = "red", 
...
Feeding_Type + Sepsis + Primary_Group + Batch

```{r}
plot_richness(ps2, measures = c("Shannon", "Observed"), 
              x = "Primary_Group",  title = "") +
              geom_boxplot() +
              geom_jitter(size = 5, alpha = 0.7, mapping = aes(color = Batch)) +
              theme(panel.border = element_rect(colour = "grey", fill = NA, size = 1), 
               axis.ticks = element_blank()) 
```
