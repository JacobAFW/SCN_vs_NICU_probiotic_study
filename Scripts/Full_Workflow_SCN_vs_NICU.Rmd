...
sample_data(ps) <- sample_data(ps) %>%
  ...
  mutate("Sample" = ID) %>% # redo rownames to save it back into the original ps object
...
plot_abundance = function(physeq, Title = "Abundance", Facet = "Order", 
                          Color = "Phylum", variable = "Type"){
...
homogeneity <- as.data.frame(anova(betadisper(dist, ps_metadata$Neonatal_Antibiotics))) %>% 
  tableGrob() %>% 
  annotate_figure(fig.lab = "Homogeneity Condition", 
...
sample_data(ps3) %>% 
  unclass %>% 
  as.data.frame() %>% 
  select(Feeding_Type, NEC, Sepsis, Mode_of_Delivery, Neonatal_Antibiotics, Chorioamnionitis, 
         Preeclampsia, ROP, Primary_Group, Batch) %>% 
  centre_and_scale() %>% 
  corvif()
```
...
phyloseq_to_deseq2(ps3, ~ Feeding_Type + NEC + Sepsis + Mode_of_Delivery +  
                   Neonatal_Antibiotics + Chorioamnionitis + Preeclampsia + 
                   ROP + Primary_Group + Batch) %>% 
    calc_geo_means() %>% 
    deseq_filter(10, 10) %>% 
...
phyloseq_to_deseq2(ps3, ~ Primary_Group + Feeding_Type + NEC + Sepsis + Mode_of_Delivery + 
                     Neonatal_Antibiotics + Chorioamnionitis +
...
### Wald for all vairables in the model 
...
deseq_model %>% 
  get_deseq_res_cat("Primary_Group", "NICU", "SCN") %>% 
  remove_rownames() %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Feeding_Type", "Formula", "Breastmilk") %>% 
          remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("NEC", "Yes", "No") %>% 
          remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Sepsis", "Yes", "No") %>% 
          remove_rownames()) %>% 
   rbind(deseq_model %>% get_deseq_res_cat("Mode_of_Delivery", "Vaginal", "Cesarean") %>% 
           remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Diabetes", "Yes", "No") %>% 
          remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Neonatal_Antibiotics", "Yes", "No") %>% 
          remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Preeclampsia", "Yes", "No") %>% 
          remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Chorioamnionitis", "Yes", "No") %>% 
          remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Batch", "Run1", "Run2") %>% 
          remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("ROP", "Yes", "No") %>% 
          remove_rownames()) %>% 
  rbind(deseq_model %>% get_deseq_res_cat("Antenatal_Antibiotics", "Yes", "No") %>% 
          remove_rownames())
```
...
```{r,eval=F}
phyloseq_to_deseq2(ps3, ~ Primary_Group + Feeding_Type + NEC + Sepsis + 
...
permanova <- adonis(ps_otu ~Primary_Group + Feeding_Type + NEC + Sepsis + 
                      Mode_of_Delivery +  Neonatal_Antibiotics + Chorioamnionitis + 
                      Preeclampsia + ROP + Batch, data = ps_metadata, 
...
ordination_distance <- ordinate(
    ...
    formula = ~ Primary_Group + Feeding_Type + NEC + Sepsis + Mode_of_Delivery + 
      ...
      Gestational_Age_at_Birth + Prolonged_Membrane_Rupture + Diabetes + Birth_Weight + 
 ...
 envdat.phy <- ps2.TSS %>% 
  ...
  unclass() %>% 
  as.data.frame() %>% 
  select(Primary_Group, Feeding_Type, NEC, Sepsis, Mode_of_Delivery, Neonatal_Antibiotics, 
         Chorioamnionitis, Preeclampsia, ROP, Batch, Gestational_Age_at_Birth, 
         Prolonged_Membrane_Rupture, Diabetes, Birth_Weight, 
         Nursery_Discharge_Weight, Antenatal_Infections, Antenatal_Antibiotics) 
...
envfit_object$vectors$r %>% 
  ...
  rownames_to_column("Variable") %>%  
  rename("r2" = ".") %>% 
  rows_update(., tibble(Variable = "Sepsis", r2 = 0.1)) %>% # 0.1 to fit plot 
  left_join(
...
### Only variables previously found to have significant associations
...
envdat.phy <- ps2.TSS %>% 
  ...
  as.data.frame() %>% 
  select(Primary_Group, Feeding_Type, NEC, Sepsis, Mode_of_Delivery, Neonatal_Antibiotics, 
         Chorioamnionitis, Preeclampsia, ROP, Batch, Gestational_Age_at_Birth, Diabetes, 
         Antenatal_Antibiotics) %>% 
  centre_and_scale()
...
major_contributors(ps2.TSS, "Primary_Group") %>% # ASVs from the same genus - need to summarise 
  filter(top.coef > 0) %>% 
  group_by(Genus) %>% 
...
grid.arrange(
  ...
  rownames_to_column("Variable") %>%  
  rename("r2" = ".") %>% 
  rows_update(., tibble(Variable = "Sepsis", r2 = 0.1)) %>%
  left_join(
    envfit_object$factors$pvals %>% 
  as.data.frame() %>% 
  rownames_to_column("Variable") %>%  
  ...
  theme(axis.line =  element_line(size = .5), panel.background = element_blank(), 
    ...
    scale_y_discrete(labels = c("Primary_Group" = "Probiotic group",
                              ...
                              "Feeding_Type" = "Diet",
                              "Antenatal_Antibiotics" = "Antenatal antibiotics")),
  major_contributors(ps2.TSS, "Primary_Group") %>% # ASVs from same genus - summarise
  filter(top.coef > 0) %>% 
  group_by(Genus) %>% 
  summarise("top.coef" = mean(top.coef)) %>% 
  rbind(
...
```{r, warning=F, message=F, results='hide',eval=F}
global <- lme4::glmer(Shannon ~ Feeding_Type + Sepsis + Mode_of_Delivery + Preeclampsia + 
                        ROP + Primary_Group + Batch + (1|URN), data = glm_data) 

global <- lme4::glmer(Shannon ~ Feeding_Type + Sepsis + Mode_of_Delivery + Preeclampsia + 
                        ROP + Primary_Group + Batch + (1|URN), 
                      data = (glm_data %>% filter(Shannon > 0)), family = Gamma(link = "log"))

global <- lme4::glmer(Shannon ~ Primary_Group + Feeding_Type + NEC + Sepsis + Mode_of_Delivery + 
                        Neonatal_Antibiotics + Chorioamnionitis + Preeclampsia + ROP + Batch + 
                        Prolonged_Membrane_Rupture + Diabetes + Nursery_Discharge_Weight + 
                        Antenatal_Infections + Antenatal_Antibiotics + (1|URN), 
                      data = glm_data %>% filter(Shannon > 0), family = Gamma(link = "log")) 
```
...
global2 <- lme4::glmer(Shannon ~ Feeding_Type + Sepsis + Primary_Group + Batch + (1|URN), 
                       data = glm_data) 
...
```{r,eval=F}
lmerTest::lmer(Shannon ~ Feeding_Type + Sepsis + Primary_Group + Batch + (1|URN), 
               data = glm_data) %>% summary()
```
